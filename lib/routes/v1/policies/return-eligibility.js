import { compute, getRulesVersion } from "../../../return-compute.js";
...
export default async function handler(req, res) {
  ...
  if (!rateLimitResult.allowed) {
    ...
    await persistLog('return_request', { request_id, event: 'rate_limit_exceeded', ip: clientIp, ua });
    return;
  }
  ...
  try {
    if (req.method === "GET") {
      return json(res, 200, {
        ok: true,
        service: "return.decide.fyi",
        request_id,
        message: "Use POST with JSON body.",
        endpoint: "/api/v1/return/eligibility",
    ...
    await persistLog('return_request', {
      request_id,
      method: req.method,
      vendor: body?.vendor ?? null,
