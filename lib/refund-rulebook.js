...
export const REFUND_POLICY_RULEBOOK = Object.freeze(
  JSON.parse(readFileSync(new URL("../rules/refund-policy-notary-v1.json", import.meta.url), "utf8"))
);

export function evaluateRefundPolicyRulebook(inputs) {
  const evaluation = evaluateRulebookV1({
    rulebook: REFUND_POLICY_RULEBOOK,
    inputs,
  });
  if (!evaluation.ok) {
    throw new Error(`Refund policy rulebook invalid: ${evaluation.error}`);
  }
  ...
  return attachPolicyDecisionMaterial(
    ...
    { rulebook: REFUND_POLICY_RULEBOOK, inputs }
  );
}
